...
#   project_dir — path to the project root; defaults to ../ (parent of .agent-modeling-kit)
...
if [[ ! -f "$KIT_DIR/.eventmodelers/config.json" ]]; then
  ...
  echo "Run: npx @eventmodelers/agent-modeling-kit install"
  exit 1
fi
...
while [[ "$ITERATIONS" -eq 0 || "$cycle" -lt "$ITERATIONS" ]]; do
  if has_pending_tasks; then
    run_agent "Process the next task from .agent-modeling-kit/tasks.json."
  else
    sleep 5
  fi
  (( cycle++ )) || true
