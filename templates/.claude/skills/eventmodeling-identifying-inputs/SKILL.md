...
Instead of writing a markdown document, **place each COMMAND on the board** using the `place-element` skill.

For each command identified, invoke `place-element` with:
...
| `position` | column index of the related EVENT node (same column) |
